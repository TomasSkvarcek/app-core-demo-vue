...
class UserPasswordResetMail extends Mailable implements ShouldQueue
{
    ...
    {
        $this->password_reset_link = url(UrlPaths::PASSWORD_RESET.'?'.Arr::query(['token' => $password_reset_token, 'id' => $hashed_email]));
    }

    public function envelope(): Envelope
    {
        return new Envelope(
            subject: __('email.subject.password_reset', ['app_name' => config('app.name')]),
        );
    }
    ...
    {
        return new Content(
            view: 'user.mail.'.App::currentLocale().'.password-reset',
        );
    }
}
